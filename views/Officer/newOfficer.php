    ...
    <div class="container">
        <br>
            <form method = "get"action="">
                <label>Agency Name: <br>
                    <select name="districtAgencyID" class="dropdown-menu position-static d-grid gap-1 p-2 rounded-3 mx-0 border-0 shadow w-220px">
                        <?php foreach($agencyList as $agency){
                            echo "<option value=$agency->districtAgencyID>$agency->agencyName,$agency->districtName,$agency->provinceName</option>";
                        }?>
                    </select>
                </label><br>
                <label><br>Title : <br>
            ...
            <?php foreach($enumList as $enum){
                ...
                <label>LastName : 
                    <input input type="text" name="lastName" class="form-control"/>
                </label><br><br>
                ...
                <input type="hidden" name="controller" value="Officer"/>
                        ...
                        <button type="submit" name="action" value="addOfficer" class='btn btn-sm btn-outline-secondary'>Create</button> 
...
<!-- <br>
<form method = "get"action="">
    <label>Agency Name<select name="districtAgencyID">
        <?php //foreach($agencyList as $agency){
            //echo "<option value=$agency->districtAgencyID>$agency->agencyName,$agency->districtName,$agency->provinceName</option>";
        //}?>
        </select>
    </label><br>
    <label>Title<select name="enum">
            <?php //foreach($enumList as $enum){
                //echo "<option value = $enum> $enum</option>";
            //}?>
    </select>
    ...
    <button type="submit" name="action" value="addOfficer">Save</button> -->